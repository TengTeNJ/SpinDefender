...
import 'ble_frame_parse.dart';
import 'ble_send_data.dart';

typedef DeviceStatusCallback = void Function(DeviceStatus status);
...
class BluetoothManager {
  ...
  late final BleFrameAssembler _assembler;
  ...
  Future<void> startNewScan() async {
    ...
    _assembler = BleFrameAssembler(
      onParsedFrame: (frame) {
        if (frame.cmd != BleResponseCmd.status) {
          BluetoothManager().sendDataToSimpleDevice(
            buildFrame(cmd: SpinDefenderCmd.requestStatus.value),
          );
        }
        switch (frame.cmd) {
          case BleResponseCmd.start:
            print('收到启动指令');
            break;

          case BleResponseCmd.stop:
            print('收到停止指令');
            break;

          case BleResponseCmd.setSpeed:
            final speed = frame.data.first;
            print('设置速度: $speed');
            break;

          case BleResponseCmd.status:
            print('状态上报: ${frame.data}');
            final status = DeviceStatus.fromBleData(frame.data);
            deviceStatus = status;
            // ⭐ 通知页面
    ...
    _bleListen = _scanStream!.listen((DiscoveredDevice event) {
      ...
      if (!hasDevice(event.id)) {
        ...
        if (conectedDeviceCount.value == 0 &&
            model.device.name == kBLEDevice_NewName) {
          // 已经连接的设备少于两个 则自动连接
  ...
  sendDataToSimpleDevice(List<int> data) {
    if (BluetoothManager().hasConnectedDeviceList.isEmpty) return;
    BLEModel model = BluetoothManager().hasConnectedDeviceList[0];
    writerDataToDevice(model, data);
  }
